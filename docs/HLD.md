...
// 유일하게 User에 매달리지 않는 모델 — 로그인 전 요청을 세는 게 목적이므로.
model RateLimit {
  key         String   @id      // login:email:… | login:ip:… | signup:ip:…
  count       Int      @default(0)
  windowStart DateTime @default(now())

  @@index([windowStart])        // 만료 행 정리용
}
```
...
- **속도 제한 (`lib/rate-limit.ts`)**: 로그인은 이메일당 10회/15분 + IP당 30회/15분(**실패만** 집계, 성공 시 해당 이메일 카운트 삭제), 가입은 IP당 5개/시간. 카운터는 **DB에 둔다** — Vercel 서버리스에서 인메모리 카운터는 인스턴스마다 따로 놀고 콜드 스타트마다 리셋돼 사실상 아무것도 막지 못한다.
  - **입력된 이메일 기준으로 집계한다(계정 존재 여부와 무관)** — 존재하는 계정만 세면 잠금 여부가 계정 존재를 알려주는 오라클이 된다
  - 잠금은 비밀번호 검증 **전에** 확인해 bcrypt 비용도 물리지 않는다. 잠긴 동안엔 올바른 비밀번호도 거부된다
  - 고정 윈도(sliding 아님)라 경계에 걸친 버스트는 최대 2배까지 통과할 수 있다 — 무제한 추측을 15분당 수십 회로 낮추는 게 목적이라 허용
  - 잠금은 `CredentialsSignin` 하위 클래스의 `code: "rate_limited"`로 UI에 전달돼 "비밀번호가 틀렸다"가 아니라 잠금이라고 설명한다
  - 만료 행은 크론 없이 쓰기 트래픽에 편승해(약 1/50 확률) 정리한다
